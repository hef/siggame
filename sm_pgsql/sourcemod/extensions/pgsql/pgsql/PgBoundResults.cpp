...
//@TODO: Fix Me
PgBoundResults::PgBoundResults(/*MYSQL_STMT *stmt, MYSQL_RES *res*/)
...
bool PgBoundResults::Initialize()
{
...
unsigned int PgBoundResults::GetRowCount()
{
	return m_RowCount;
}

unsigned int PgBoundResults::GetFieldCount()
{
	return m_ColCount;
}

const char *PgBoundResults::FieldNumToName(unsigned int columnId)
{
...
bool PgBoundResults::FieldNameToNum(const char *name, unsigned int *columnId)
{
	for (unsigned int i=0; i<m_ColCount; i++)
	{
...
bool PgBoundResults::MoreRows()
{
	return (m_CurRow < m_RowCount);
}

IResultRow *PgBoundResults::FetchRow()
{
	if (!MoreRows())
	{
...
IResultRow *PgBoundResults::CurrentRow()
{
...
DBType PgBoundResults::GetFieldType(unsigned int field)
{
	if (field >= m_ColCount)
	{
...
DBType PgBoundResults::GetFieldDataType(unsigned int field)
{
...
//@TODO: Fix Me
bool PgBoundResults::RefetchField(/*MYSQL_STMT *stmt, */
				  unsigned int id,
				  size_t initSize /*,
...
DBResult PgBoundResults::GetString(unsigned int id, const char **pString, size_t *length)
{
	BAD_COL_CHECK();
...
DBResult PgBoundResults::CopyString(unsigned int id, char *buffer, size_t maxlength, size_t *written)
{
	BAD_COL_CHECK();
...
DBResult PgBoundResults::GetFloat(unsigned int id, float *pFloat)
{
	BAD_COL_CHECK();
...
DBResult PgBoundResults::GetInt(unsigned int id, int *pInt)
{
	BAD_COL_CHECK();
...
bool PgBoundResults::IsNull(unsigned int id)
{
	if (id >= m_ColCount)
	{
		return true;
...
DBResult PgBoundResults::GetBlob(unsigned int id, const void **pData, size_t *length)
{
	BAD_COL_CHECK();
...
DBResult PgBoundResults::CopyBlob(unsigned int id, void *buffer, size_t maxlength, size_t *written)
{
	BAD_COL_CHECK();
...
size_t PgBoundResults::GetDataSize(unsigned int id)
{
	if (id >= m_ColCount)
	{
		return 0;
